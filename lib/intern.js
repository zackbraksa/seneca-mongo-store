...
function fixquery(q, seneca, options) {
  ...
  for (const qp in q) {
    ...
    if (is_mongo_operator(qp)) {
      ...
      seneca.log.warn(
        "Passing MongoDB operators directly via the query" +
...
function is_mongo_duplicate_key_error(err) {
  return err && 11000 === err.code && "DuplicateKey" === err.codeName;
}
